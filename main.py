...
def get_database_url():
    """
    Используем SQLite с хранением в Volume
    Railway Volume монтируется в /data
    """
    # Проверяем, есть ли Volume (на Railway)
    volume_path = '/data'
    if os.path.exists(volume_path) and os.access(volume_path, os.W_OK):
        db_path = os.path.join(volume_path, 'bot.db')
        logger.info(f"📁 Используем Volume: {db_path}")
        return f'sqlite:///{db_path}'
    
    # Если Volume нет, используем локальную папку
    db_path = os.path.join(os.getcwd(), 'bot.db')
    logger.info(f"📁 Используем локальную БД: {db_path}")
    return f'sqlite:///{db_path}'
